<?php
    ...
    $query = "SELECT camera_id, species, COUNT(*) AS count FROM wild_animal2 GROUP BY camera_id, species ORDER BY camera_id, species";
    ...
    if($result) {
        while($row = mysqli_fetch_assoc($result)) {
            ...
            $count_result = mysqli_query($conn, "SELECT SUM(count) AS total_count FROM wild_animal2 WHERE camera_id = '$camera_id' AND species = '$species'");
...
<?php
    ...
    // createdAt 역순으로 정렬
    $query = "SELECT * FROM wild_animal2 ORDER BY createdAt DESC LIMIT 10";
    $result = mysqli_query($conn, $query);
    // 데이터베이스 연결 해제
    mysqli_close($conn);
?>
